...
class GaoGraphBasic():
    ...
    def get_relationship(self, as1, as2):
        try:
            return self.edges[(as1, as2)]
        except KeyError:
            pass

        try:
            inv_etype = self.edges[(as2, as1)]
            if inv_etype == CUSTOMER_TO_PROVIDER:
                return PROVIDER_TO_CUSTOMER
            elif inv_etype == PROVIDER_TO_CUSTOMER:
    ...
    def get_classified_edges(self):
        transit = self._phase2(self.neighbors)
...
class ASGraph(GaoGraphHeuristic):

    def __init__(self, paths_file):
        paths = get_paths_from_file(paths_file)
